...
def sendMsg():
    ...
    if resp["errcode"] != 0:
        print "Error {0}: {1}".format(resp["errcode"], resp["errmsg"])

        if resp["errcode"] == 42001 or resp["errcode"] == 40014:
            access_token = getToken()
            print requests.post("https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token={0}".format(access_token), data = params).text
